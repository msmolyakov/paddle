...
Paddle не зависит от тестового фреймворка. Вы может использовать Paddle совместно с JUnit, TestNG или любым другим фреймворком.
...
На момент старта ноды все токены Waves лежат на специальном майнящем аккаунте `rich`.\
...
- `node.api.assetDetails(assetId)` - информация о выпущенном ассете;
...
Также _комиссия транзакции тоже определяется автоматически_! Единственное, на данный момент не рассчитываются комиссии в спонсорском ассете или если в результате InvokeScript транзакции передаются смарт токены.
...
```
Теперь транзакции будут отправлены в три потока, а последовательно будут выполняться только зависимые транзакции.
